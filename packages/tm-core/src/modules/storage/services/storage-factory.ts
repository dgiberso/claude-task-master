...
import { GitHubStorage } from '../adapters/github-storage.js';
...
export class StorageFactory {
        ...
        static async create(
                ...
                switch (storageType) {
                        ...
                        case 'github':
                                logger.info('🐙 Using GitHub Issues storage');
                                return StorageFactory.createGitHubStorage(config);
        ...
        /**
         * Create GitHub storage implementation
         */
        private static createGitHubStorage(
                config: Partial<IConfiguration>
        ): GitHubStorage {
                const token = config.storage?.githubToken || process.env.GITHUB_TOKEN;
                const owner = config.storage?.githubOwner;
                const repo = config.storage?.githubRepo;

                if (!token) {
                        throw new TaskMasterError(
                                'GitHub token is required. Set GITHUB_TOKEN environment variable or configure in storage settings.',
                                ERROR_CODES.MISSING_CONFIGURATION,
                                { storageType: 'github' }
                        );
                }
                ...
                if (!owner || !repo) {
                        throw new TaskMasterError(
                                'GitHub repository owner and name are required for GitHub storage.',
                                ERROR_CODES.MISSING_CONFIGURATION,
                                { storageType: 'github', owner, repo }
                        );
                }
                ...
                return new GitHubStorage({
                        ...
                        owner,
                        repo,
                        defaultLabels: config.storage?.githubDefaultLabels || ['taskmaster'],
        ...
        static detectOptimalStorage(
                ...
                // If GitHub credentials are provided, prefer GitHub storage
                if (
                        config.storage?.githubToken &&
                        config.storage?.githubOwner &&
                        config.storage?.githubRepo
                ) {
                        return 'github';
                }
        ...
        static validateStorageConfig(config: Partial<IConfiguration>): {
                ...
                switch (storageType) {
                        ...
                        case 'github':
                                if (!config.storage?.githubToken) {
                                        errors.push('GitHub token is required for GitHub storage');
                                }
                                if (!config.storage?.githubOwner) {
                                        errors.push('GitHub repository owner is required for GitHub storage');
                                }
                                if (!config.storage?.githubRepo) {
                                        errors.push('GitHub repository name is required for GitHub storage');
                                }
                                break;
